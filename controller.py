...
def uebersicht():
    ...
    meldungen = admin_1.get_alle_meldungen() if not eigene else student_1.meldungen
    
    #Filter
    if selected_modul:
        meldungen = [meldung for meldung in meldungen if str(meldung.modul.id) == selected_modul]
    if selected_status:
        meldungen = [meldung for meldung in meldungen if meldung.status.name == selected_status]
    if selected_kategorie:
        meldungen = [meldung for meldung in meldungen if meldung.kategorie.name == selected_kategorie]
